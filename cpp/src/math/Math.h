...
inline simd::float4x4 ortho_rh(float left, float right, float bottom, float top, float near, float far)
{
...
inline simd::float4x4 model_ts(simd::float3 translation, simd::float3 scale = simd::float3{1.0f, 1.0f, 1.0f})
{
    return simd::float4x4{
